...
const PROMPT_VERSION = "myspmcare-intervention-v2";
...
function hasStudentName(student) {
...
function replaceStudentCodeWithName(value, student) {
  ...
  if (!student.studentCode || !hasStudentName(student)) return text;

  const displayName = student.name;
  const codePattern = escapeRegExp(student.studentCode);
  text = text.replace(new RegExp(`\\s*\\(${codePattern}\\)`, "g"), ` ${displayName}`);
  text = text.replace(new RegExp(codePattern, "g"), displayName);
  return text.trim();
}
...
function buildGeminiPrompt(student) {
  const safeProfile = {
    studentName: hasStudentName(student) ? student.name : "Murid ini",
  ...
  return [
    ...
    "Gunakan nama murid daripada studentName apabila merujuk murid. Jangan paparkan kod murid atau student_code dalam ayat cadangan.",
