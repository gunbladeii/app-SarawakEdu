...
const ORACLE_RETRY_DELAYS_MS = [0, 800, 1800];
const DEFAULT_STUDENT_BATCH_LIMIT = 250;
...
function getRequestTimeoutMs() {
  return clampInteger(env("ENAZIR_ORACLE_TIMEOUT_MS"), DEFAULT_REQUEST_TIMEOUT_MS, 5000, 55000);
}
...
function shouldRetryOracleError(error) {
  if (!(error instanceof ApiError)) {
    return true;
  }

  return error.status === 502 || error.status === 504 || error.status === 408 || error.status >= 500;
}

function logOracleWarning(endpoint, error, attempt) {
  const detail = {
    endpoint,
    attempt,
  ...
  console.warn("[Oracle reference]", detail);
}

async function fetchOracleOnce(endpoint, params = {}) {
...
async function fetchOracle(endpoint, params = {}) {
  ...
  for (let attempt = 0; attempt < ORACLE_RETRY_DELAYS_MS.length; attempt += 1) {
    if (ORACLE_RETRY_DELAYS_MS[attempt] > 0) {
      await sleep(ORACLE_RETRY_DELAYS_MS[attempt]);
    }

    try {
      return await fetchOracleOnce(endpoint, params);
    } catch (error) {
      lastError = error;
      logOracleWarning(endpoint, error, attempt + 1);

      if (!shouldRetryOracleError(error) || attempt === ORACLE_RETRY_DELAYS_MS.length - 1) {
        throw error;
      }
    }
  }
...
function getMaskedIdentityHint(value) {
  const digits = String(value || "").replace(/\D/g, "");
  if (digits.length < 4) {
    return "";
  }
  return `KP akhir ${digits.slice(-4)}`;
}
...
function normalizeStudent(row, schoolCode, includeRaw = false, classMap = new Map()) {
  ...
  const identityNumber = getFirst(row, ["NOKP", "NO_KP", "NOIC", "KP", "KPUTAMA"]);
  const student = {
    ...
    identityHint: getMaskedIdentityHint(identityNumber),
...
async function fetchStudentRowsForSchool(schoolCode, options) {
  const requestedLimit = clampInteger(options.limit, DEFAULT_PAGE_LIMIT, 1, DEFAULT_PAGE_LIMIT);
  const batchLimit = Math.min(
    requestedLimit,
    clampInteger(env("ENAZIR_STUDENT_BATCH_LIMIT"), DEFAULT_STUDENT_BATCH_LIMIT, 50, 500)
  );

  return fetchOracleAll("murid", {
...
async function handleStudents(searchParams) {
  ...
  const chunks = await Promise.all(
    selectedSchools.map(async (school) => {
      ...
      const rows = await fetchStudentRowsForSchool(school.code, {
        limit,
        offset,
