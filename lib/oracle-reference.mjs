...
async function handleStudents(searchParams) {
  ...
  const chunks = await Promise.all(
    selectedSchools.map(async (school) => {
      const { map: classMap } = await fetchClassMap(school.code, false);
...
export async function handleOracleReferenceRequest(request, response) {
  ...
  try {
    ...
    if (scope === "classes") {
      sendJson(response, 200, await handleClasses(url.searchParams));
      return;
    }

    if (scope === "enrolment") {
      sendJson(response, 200, await handleEnrolment(url.searchParams));
      return;
    }
